...
public interface PlaceApi {
    ...
    @Operation(summary = "장소 목록", description = "노출 중인 장소만 표시 순서와 이름순으로 반환한다. 카테고리는 STAGE(공연장), TOILET(화장실), DELIVERY_ZONE(배달존)만 지원한다. 사용자의 현재 위치가 있으면 프론트가 거리순으로 재정렬한다. 로그인 불필요. FR-MAP-01, 02, 06")
    @GetMapping
    ApiResponse<List<PlaceListResponse>> getPlaces(
            @Parameter(description = "장소 카테고리(STAGE: 공연장 / TOILET: 화장실 / DELIVERY_ZONE: 배달존)") @RequestParam(required = false) PlaceCategory category
    );
