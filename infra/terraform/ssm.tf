...
# JWT_SECRET, KAKAO_*, GOOGLE_* 5개는 사용자가 CLI로 직접 넣는다(README). DB 비밀번호만 RDS 생성에 필요해 변수로 받아 같은 값을 파라미터로 만든다
...
locals {
  ...
  user_managed_secrets = ["JWT_SECRET", "KAKAO_CLIENT_ID", "KAKAO_CLIENT_SECRET", "GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET"]
