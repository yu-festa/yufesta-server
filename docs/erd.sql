...
-- 운영자: 별도 테이블 없이 users.role(STAFF/OWNER)로 구분. role은 사용자 API로 변경 불가
...
INSERT INTO `app_settings` (`setting_key`, `setting_value`, `description`) VALUES
  ('match.weight.tag',              '1',   '공통 태그 1개당 점수'),
  ('match.weight.slot',             '2',   '같은 공연 선택 점수'),
  ('match.weight.age_same',         '1',   '나이대 동일 점수'),
  ('match.weight.age_adjacent',     '0.5', '나이대 인접 점수'),
  ('match.max_partners',            '3',   '소수 측 1인당 최대 배정 수(N)'),
  ('report.block_threshold',        '2',   '매칭 신고 누적 제재 기준'),
  ('report.hide_threshold',         '2',   '콘텐츠 신고 누적 자동 숨김 기준'),
  ('nickname.adjectives',           '수줍은,신난,느긋한,씩씩한,조용한', '자동 닉네임 형용사 목록'),
  ('nickname.animals',              '펭귄,수달,고양이,판다,돌고래',   '자동 닉네임 동물 목록'),
  ('filter.banned_words',           '',    '금칙어 목록(쉼표 구분)'),
  ('filter.llm.enabled',            '1',   'LLM 모더레이션 API 사용 여부'),
  ('filter.llm.threshold',          '0.5', '모더레이션 카테고리 차단 임계값(0~1)'),
  ('ratelimit.cheer.anon_per_minute', '1', '응원 메시지 익명 키당 분당 작성 수'),
  ('ratelimit.cheer.ip_per_minute', '10',  '응원 메시지 IP당 분당 작성 수'),
  ('ratelimit.lostitem.per_minute', '1',   '분실물 회원당 분당 작성 수'),
  ('admin.allowlist',               'KAKAO:1234567890,GOOGLE:1098765432', '운영자로 승격할 소셜 계정(PROVIDER:id). 최초 로그인 시 role 부여');

-- 초기 회차 예시 (open_at 1회차 = 사전 오픈 시각, 총동연 협의 후 운영자 화면에서 수정)
INSERT INTO `match_rounds` (`seq`, `open_at`, `close_at`, `publish_at`, `status`) VALUES
  (1, '2026-10-01 00:00:00', '2026-10-08 15:50:00', '2026-10-08 16:00:00', 'SCHEDULED'),
  (2, '2026-10-08 16:00:00', '2026-10-08 19:50:00', '2026-10-08 20:00:00', 'SCHEDULED');
