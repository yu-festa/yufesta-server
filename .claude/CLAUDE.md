...
- 컨트롤러: `@WebMvcTest(controllers = X.class)` + `@MockitoBean` 서비스. principal이 `Long`이라 `@WithMockUser`로는 `userId`가 null이 된다. `src/test/java/com/yufesta/support/WithMockLoginUser`(`WithSecurityContextFactory` 구현, `id`·`role` 지정)를 만들어 쓴다.
...
1. SecurityConfig가 `anyRequest().permitAll()` — 7장 목표 규칙으로 교체, 401/403 JSON `exceptionHandling` 추가
2. CORS 설정 없음 — `FRONTEND_URL` 오리진, `allowCredentials=true`, 헤더 `Content-Type, X-XSRF-TOKEN`
3. 로그인 성공 후 `redirect`가 API 서버 상대 경로로 감 — `FRONTEND_URL + 경로`로 리다이렉트(상대 경로 검증은 유지). `.env.example`의 `FRONTEND_URL`이 yml에 연결되어 있지 않음
